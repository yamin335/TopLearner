...
class MyCourseViewModel @Inject constructor(
    ...
    fun getPendingMyCourses(): LiveData<List<CreateOrderBody>> {
        val pendingMyCourses = MutableLiveData<List<CreateOrderBody>>()
        try {
            ...
            viewModelScope.launch(handler) {
                pendingMyCourses.postValue(pendingMyCourseDao.getAllPendingCourses())
            }
        } catch (e: SQLiteException) {
            e.printStackTrace()
        }
        return pendingMyCourses
    }
    ...
    fun getMyCourseBookListFromDB(bookIds: List<Int>): LiveData<List<MyCourseBook>> {
        val bookList = MutableLiveData<List<MyCourseBook>>()
        try {
            val handler = CoroutineExceptionHandler { _, exception ->
                exception.printStackTrace()
            }
            ...
            viewModelScope.launch(handler) {
                bookList.postValue(myCourseDao.getMyCourseBookList(bookIds))
            }
        } catch (e: SQLiteException) {
            e.printStackTrace()
        }
